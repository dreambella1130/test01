...
-- 회원정보 테이블 컬럼추가
ALTER TABLE MEM_INFO ADD(MEM_NAME VARCHAR2(30));
ALTER TABLE MEM_INFO ADD(MEM_BIRTH DATE);

DESC MEM_INFO;

-- 회원정보 입력
INSERT INTO MEM_INFO(MEM_SID, MEM_NAME, MEM_BIRTH, MEM_ID, MEM_PW, MEM_NICK, MEM_REGI, MEM_UPDT, MEM_BL, MEM_GRADE)
VALUES(SEQ_MEM_INFO.NEXTVAL, '홍길동', TO_DATE('20170620', 'YYYYMMDD'), 'bellabella1130@naver.com', '1111@', 'MOON', SYSDATE, SYSDATE, 'N', '9');

SELECT MEM_SID, MEM_NAME, MEM_BIRTH, MEM_ID, MEM_PW, MEM_NICK, MEM_REGI, MEM_UPDT, MEM_BL, MEM_GRADE
FROM MEM_INFO;
...
(
    ...
    FROM BOARD A
    WHERE BD_DELCK = 'N'
    AND ( UPPER(REGEXP_REPLACE(BD_TITLE,'<[^>]*>|\&([^;])*;', '')) LIKE '%'||UPPER('ddd')||'%'
        OR UPPER(REGEXP_REPLACE(BD_CONT,'<[^>]*>|\&([^;])*;', '')) LIKE '%'||UPPER('ddd')||'%' )
)
